...
const WatchLater = () => { 
  ...
  useEffect(() => {
    const fetchWatchLater = async () => {
      try {
        const response = await fetch(`movie-api-blush.vercel.app/api/${userId}/watchlater`);
  ...
  const removeFromWatchLater = async (movieId) => {
    try {
      const response = await fetch(`movie-api-blush.vercel.app/api/${userId}/watchlater/${movieId}`, {
        method: 'DELETE',
      });
      const data = await response.json();
      console.log(data);
