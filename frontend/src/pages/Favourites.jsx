...
const Favourites = () => {
  ...
  useEffect(() => {
    const fetchFavourites = async () => {
      try {
        const response = await fetch(`https://movie-flax-ten.vercel.app/api/${userId}/favourites`);
  ...
  const removeFromFavourites = async (movieId) => {
    try {
      const response = await fetch(`https://movie-flax-ten.vercel.app/api/${userId}/favourites/${movieId}`, {
        method: 'DELETE',
      });
      const data = await response.json();
      console.log(data);
