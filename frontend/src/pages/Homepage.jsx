...
const Homepage = () => {
  ...
  const PopularMovies = async()=>{
    const response = await fetch(`https://movie-flax-ten.vercel.app/api/movies/popular`)
  ...
  const fetchLatestTvShows = async () => {
      const response = await fetch(`https://movie-flax-ten.vercel.app/api/tvshows/on_the_air`);
      const data = await response.json();
      setLatestTvShows(data.results.slice(0,12));
  }

  const fetchTopRatedTvShows = async () => {
    const response = await fetch(`https://movie-flax-ten.vercel.app/api/tvshows/top_rated`);
    const data = await response.json();
